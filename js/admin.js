...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (saveSettingsBtn) {
        saveSettingsBtn.addEventListener('click', async () => {
            // sanitizeToken() dari storage.js: buang spasi/enter yang tak sengaja ikut ter-paste
            const tokenVal = configToken ? sanitizeToken(configToken.value) : '';
            ...
            // Proses cek ke Github — sekarang mengembalikan alasan gagal yang detail
            const result = await verifyGitHubToken(tokenVal);
            ...
            // Tampilkan token yang sudah dibersihkan agar field tidak menyimpan sisa karakter lama
            if (configToken) configToken.value = tokenVal;
    ...
    saveFormBtn.addEventListener('click', async () => {
        ...
        // Simpan salinan sebelum diubah, untuk dikembalikan jika save gagal
        const previousData = ordnerData.map(item => ({ ...item }));
        ...
        const result = await saveData(ordnerData);

        saveFormBtn.textContent = originalText;
        saveFormBtn.disabled = false;

        if (!result.success) {
            // Gagal simpan ke GitHub: batalkan perubahan lokal supaya data
            // tidak "kelihatan tersimpan" padahal sebenarnya belum.
            ordnerData = previousData;
            renderTable(ordnerData);
            return; // Modal tetap terbuka agar pengguna bisa coba lagi
        }
        ...
        deleteBtns.forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                if (confirm('Apakah Anda yakin ingin menghapus ordner ini secara permanen?')) {
                    ...
                    if (itemIndex > -1) {
                        const removedItem = ordnerData[itemIndex];
                        ordnerData.splice(itemIndex, 1);

                        const result = await saveData(ordnerData);

                        if (!result.success) {
                            // Gagal hapus di GitHub: kembalikan item ke posisi semula
                            ordnerData.splice(itemIndex, 0, removedItem);
                            renderTable(ordnerData);
                            return;
                        }
            ...
            reader.onload = async function(event) {
                try {
                    ...
                    if(confirm("Peringatan: Data di GitHub akan DITIMPA dengan file ini. Lanjutkan?")) {
                        const result = await saveData(importedData);
                        if (!result.success) return; // pesan error sudah ditampilkan oleh saveData
                        ordnerData = await loadData();
                        renderTable(ordnerData);
                        closeModal(modalSettings);
                    }
    ...
    if (btnReset) {
        btnReset.addEventListener('click', async () => {
            if(confirm("PERINGATAN KERAS: Seluruh data Anda akan dihapus dan diganti dengan 1 data contoh. Lanjutkan?")) {
                ...
                const result = await saveData(dummyData);
                if (!result.success) return; // pesan error sudah ditampilkan oleh saveData
                ordnerData = await loadData();
                renderTable(ordnerData);
                closeModal(modalSettings);
